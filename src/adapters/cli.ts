...
async function listAppointmentsHandler() {
  ...
  const dataToDisplay = appointments.map((appt: Appointment) => ({
    ID: appt.id,
...
async function addAppointmentHandler(args: string[]) {

  if (args.length !== 4) {
    throw new Error(
      'Uso incorreto. Formato: adicionar_compromisso "dd/mm/aaaa" "hh:mm_inicio" "hh:mm_fim" "descricao"'
    );
  }

  const [dateStr, startTimeStr, endTimeStr, description] = args as [string, string, string, string];
  ...
  const newAppt = await schedulerService.addAppointment(
    startDatetimeUTC,
    endDatetimeUTC,
    description
  );
